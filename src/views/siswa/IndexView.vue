    ...
    <div class="p-4 rounded-lg dark:border-gray-700">
      <div class="mx-auto max-w-3xl text-center">
        <h1 class="mt-5 text-3xl font-light leading-tight text-gray-900 sm:text-5xl sm:leading-tight lg:text-6xl lg:leading-tight">
         Selamat Datang Di <br class="sm:hidden" />  
          <span class="relative inline-flex justify-center whitespace-nowrap font-bold">
            ...
            </svg> Website Resmi ruangku </span >
        </h1>
        <br>
        <br>
        </div>
...
export default {
  ...
  methods: {
    seninDikerjakan() {
          ...
          const seninTugas = this.tugas_siswa.filter((element) => element.hari === "Senin" && element.id_user === id_siswa.id && element.status == 1);
          return seninTugas.length;
    },
    seninBelumDikerjakan(){
      const id_siswa =  JSON.parse(localStorage.user);
      const seninTugas = this.tugas_siswa.filter((element) => element.hari === "Senin" && element.id_user === id_siswa.id && element.status == 0);
      return seninTugas.length;
    },
    selasaDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const selasaTugas = this.tugas_siswa.filter((element) => element.hari === "Selasa" && element.id_user === id_siswa.id && element.status == 1);
          return selasaTugas.length;
    },
    selasaBelumDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const selasaTugas = this.tugas_siswa.filter((element) => element.hari === "Selasa" && element.id_user === id_siswa.id && element.status == 0);
          return selasaTugas.length;
    },
    RabuDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const rabuTugas = this.tugas_siswa.filter((element) => element.hari === "Rabu" && element.id_user === id_siswa.id && element.status == 1);
          return rabuTugas.length;
    },
    RabuBelumDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const rabuTugas = this.tugas_siswa.filter((element) => element.hari === "Rabu" && element.id_user === id_siswa.id && element.status == 0);
          return rabuTugas.length;
    },
    KamisDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const KamisTugas = this.tugas_siswa.filter((element) => element.hari === "Kamis" && element.id_user === id_siswa.id && element.status == 1);
          return KamisTugas.length;
    },
    KamisBelumDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const KamisTugas = this.tugas_siswa.filter((element) => element.hari === "Kamis" && element.id_user === id_siswa.id && element.status == 0);
          return KamisTugas.length;
    },
    JumatDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const JumatTugas = this.tugas_siswa.filter((element) => element.hari === "Jumat" && element.id_user === id_siswa.id && element.status == 1);
          return JumatTugas.length;
    },
    JumatBelumDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const JumatTugas = this.tugas_siswa.filter((element) => element.hari === "Jumat" && element.id_user === id_siswa.id && element.status == 0);
          return JumatTugas.length;
    },
    SabtuDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const SabtuTugas = this.tugas_siswa.filter((element) => element.hari === "Sabtu" && element.id_user === id_siswa.id && element.status == 1);
          return SabtuTugas.length;
    },
    SabtuBelumDikerjakan() {
      const id_siswa =  JSON.parse(localStorage.user);
          const SabtuTugas = this.tugas_siswa.filter((element) => element.hari === "Sabtu" && element.id_user === id_siswa.id && element.status == 0);
          return SabtuTugas.length;
    },
